...
import { createLiveInvoice, filterLinkedRows, InvoiceCreateInput, listLiveInvoices } from "@/lib/supabase/school-data";

export const GET = withAuth("fees.view", async (_request: NextRequest, context) => {
  ...
  try {
    const rows = await listLiveInvoices(supabase);
